...
def run_regression(features, target, target_name, model, layer):
    ...
    train_r2 = metrics.r2_score(train_pred, y_train)
    print(f"Layer: {layer}, Train R2: {train_r2:.2f}, Test R2: {test_r2:.2f}, best params: {best_param}")

    # Plot the results
    filename = f"{target_name}_{layer}_{model_name}_no_pca.png"
    ...
    plt.text(global_min, global_max-5, f'Train R$^2$: {grid_search.score(X_train_scaled, y_train).round(2)}', fontsize = 9)
    plt.text(global_min, global_max-4, f'Test R$^2$: {grid_search.score(X_test_scaled, y_test).round(2)}', fontsize = 9)
...
output_dir= f'/stor/work/Wilke/luiz/tail_stability/results/tail_stability_regression_{model}_layer{layer}.csv'
...
res = run_regression(features, target, target_name, model, layer)
                    

print(res)
res.to_csv(output_dir, index=False)
